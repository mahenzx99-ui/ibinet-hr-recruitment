<div class="hidden lg:flex lg:gap-x-10">
    ...
    <a href="{{ url('/') }}"
       class="relative group text-sm inline-flex items-center gap-1 {{ request()->is('/') ? $active : $inactive }}">
        ...
        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path d="M3 10.5 12 4l9 6.5v8.5a1.5 1.5 0 0 1-1.5 1.5h-6v-6h-3v6h-6A1.5 1.5 0 0 1 3 19z" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        Beranda
    ...
    <a href="{{ url('/contact') }}"
       class="relative group text-sm inline-flex items-center gap-1 {{ request()->is('contact') ? $active : $inactive }}">
        {{-- Icon Phone --}}
        <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <path d="M5 4h3l2 5-2 1a11 11 0 0 0 5 5l1-2 5 2v3a2 2 0 0 1-2 2A16 16 0 0 1 3 6a2 2 0 0 1 2-2Z" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>
        Kontak
        <span class="absolute left-0 -bottom-1 h-[2px] w-0 bg-blue-600 group-hover:w-full transition-all duration-300"></span>
    </a>
    ...
    @if(Auth::check() && Auth::user()->is_admin)
        <a href="{{ route('admin.applications.index') }}"
           class="relative group text-sm inline-flex items-center gap-1 {{ request()->is('admin/applications') ? $active : $inactive }}">
            {{-- Icon Shield --}}
            <svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                <path d="M12 3 4.5 6v6.5A8.5 8.5 0 0 0 12 21a8.5 8.5 0 0 0 7.5-8.5V6L12 3Z" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M10 12.5 11.5 14 14.5 10.5" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            Admin Panel
            <span class="absolute left-0 -bottom-1 h-[2px] w-0 bg-blue-600 group-hover:w-full transition-all duration-300"></span>
        </a>
